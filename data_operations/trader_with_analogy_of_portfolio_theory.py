...
filename = f'5 - hedge with constant param eta={eta} capital (Buy And Hold) с поправками'
...
if __name__ == '__main__':
    ...
    while considered_date < MAX_DATE:
        ...
        cash_inner = cash * np.dot(predicted_weights, xs_for_cash)
        ...
        weights = [xs[share_idx] * weights[share_idx] for share_idx in range(shares_num)]
    ...
    if TO_SAVE_LOGS:
        with open(logs_filename, 'a+') as f:
            f.write(
                f"{log_text}, капитал - {cash_inner}, для каждой котировки: {[shares_cash_inner[-1] for shares_cash_inner in shares_cash_list]}")
    ...
    if TO_SAVE_PLOTS:
        ...
        plt.title(f"Изменение капитала для Hedge с постоянным параметром {Algorithms.ETA}")
        plt.legend()
        # plt.show()
